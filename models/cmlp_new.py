...
def prox_update(network, lam, lr, penalty):
    ...
    if penalty == 'GL':
        ...
        for i in range(lag):
            norm = torch.norm(W[:, :, :(i + 1)], dim=(0, 2), keepdim=True)
            W.data[:, :, :(i+1)] = (
                (W.data[:, :, :(i+1)] / torch.clamp(norm, min=(lr * lam * network.Att[i])))
                * torch.clamp(norm - (lr * lam * network.Att[i]), min=0.0))
    else:
        raise ValueError('unsupported penalty: %s' % penalty)


def update_Att(network):
    W = network.layers[0].weight
    hidden, p, lag = W.shape
    At = torch.norm(W, dim=(0, 1))
    At = torch.softmax(At, dim=0)
    At = 1 / At
    At = At / At[-1]
    for t in range(lag-1):
        At[t] = At[t] - At[t+1]
    network.Att = At.tolist()
...
def regularize(network, lam, penalty):
    ...
    if penalty == 'GL':
        ...
        return lam * sum([torch.sum(torch.norm(W[:, :, :(i+1)], dim=(0, 2))) * network.Att[i]
                          for i in range(lag)])
    else:
        raise ValueError('unsupported penalty: %s' % penalty)
